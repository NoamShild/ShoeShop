...
const updateOrAddToCart = async (user, productId, size, quantity) => {
    try {
        ...
        const currentCartQuantity = cartItemIndex !== -1 ? user.cart[cartItemIndex].quantity : 0;

        if (sizeObj.amount < currentCartQuantity + quantity) {
            return { 
                success: false, 
                message: `Insufficient stock. Only ${sizeObj.amount} items available for size ${size}.`, 
                cart: user.cart 
            };
        }

        if (cartItemIndex !== -1) {
            user.cart[cartItemIndex].quantity += quantity; // Update quantity if item already exists in cart
        } else {
            user.cart.push({ productId, size, quantity }); // Add new item to cart
        }
